...
本次更新为 `record_predictor.py` 添加了两个核心功能：
...
```bash
# 不指定宿主（所有记录权重相同）
python record_predictor.py -i otu_table.tsv -d record_db.tsv -o output.tsv
...
```bash
# 旧版命令（仍然有效）
python record_predictor.py -i otu.tsv -d record_db.tsv -o output.tsv
